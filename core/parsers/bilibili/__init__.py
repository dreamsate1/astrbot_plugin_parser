...
try:
    ...
    # impersonate 非空时 CurlCFFIClient 会丢弃自定义 UA，导致裸 Mozilla UA 触发
    # bilibili 对海外机房 IP 的 412 风控；置空并显式指定 UA 可稳定通过。
    import bilibili_api.utils.network as _bili_net
    _bili_net.HEADERS["User-Agent"] = "curl/8.5.0"
    request_settings.set("impersonate", "")
